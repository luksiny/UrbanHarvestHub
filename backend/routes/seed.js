...
const { Workshop, Product, Event, Admin } = require('../models');
...
router.post('/initialize', async (req, res) => {
    try {
        ...
        if (existingProducts > 0) {
            return res.json({
                success: false,
                message: 'Database already has data. To reseed, manually delete existing data first.',
                counts: {
                    products: existingProducts,
                    workshops: await Workshop.count(),
